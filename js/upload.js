  ...
  try {
    ...
    wrap.innerHTML = `
      ...
      <p style="font-size:0.8rem;color:var(--navy-60);margin-top:8px;">
        This test just went live — its questions are added automatically.
        <a href="dashboard.html#mocktests">Choose a different test instead →</a>
      </p>
    `;
  } catch (err) {
    console.error(err);
...
function currentTargetTestId() {
  return lockedTestId || $("#targetTest")?.value || "";
}
...
function setStep(stepName) {
  $all(".upload-step").forEach((el) => {
    ...
    if (order.indexOf(el.dataset.step) < order.indexOf(stepName)) {
          ...
          ${sample
            ...
            <tr>
              ${REQUIRED_COLUMNS.map(
                (c) => `<td data-label="${c}">${escapeHtml(String(r[c] ?? ""))}</td>`
              ).join("")}
            </tr>`
            )
            .join("")}
...
async function handleSave(e) {
  ...
  try {
    ...
    // ✅ Automatically keep the card's question count correct
    await updateDoc(doc(db, "tests", testId), {
      questionCount: increment(validRows.length),
    });
