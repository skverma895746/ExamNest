  ...
  try {
    ...
    for (const t of testsSnap.docs) {
      ...
      const countSnap = await getCountFromServer(
        collection(db, "tests", t.id, "questions")
      );
...
export async function loadMockTestsTable() {
  ...
  tbody.innerHTML = `<tr><td colspan="7">Loading tests...</td></tr>`;
      ...
      .map((d) => {
        ...
        <tr data-id="${d.id}">
          ...
          <td data-label="Actions" class="row-actions">
            ...
            <button class="icon-btn" data-action="duplicate">
              Duplicate
            </button>
  ...
  if (testId) {
    ...
    getDoc(
      ...
      .then((snap) => {
        ...
        // NEW: Default marks per question
        form.marksPerQuestion.value =
...
function closeTestModal() {
  $("#testModalBackdrop").classList.remove(
    "is-open"
  );
}
...
async function saveTestFromModal(e) {
  ...
  const payload = {
    ...
    // NEW: Marks awarded for one correct question
    marksPerQuestion:
      Number(form.marksPerQuestion.value) || 1,

    // Existing negative marking
    negativeMarking:
      Number(form.negativeMarking.value) || 0,
